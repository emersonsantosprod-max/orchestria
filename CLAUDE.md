...
- Distribuição contratual já migrada: `app/domain/distribuicao_contratual.py` (puro: `parse_distribuicao_cols`, `localizar_colunas_chave`, `normalizar_linhas`, `validar_distribuicao_cobranca`, constantes `ERRO_*`/`AVISO_*`) + `app/infrastructure/adapters/excel_distribuicao_contratual.py` (`ler_xlsx_contratual`, `escrever_xlsx_normalizado`, `DistribuicaoContratualMalformadaError`); composição em `app/cli/normalizar.py`. Sem service intermediário (composition root é a CLI). Legacy `app/distribuicao_contratual.py` removido.
